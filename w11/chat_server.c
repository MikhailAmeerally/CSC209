...
int accept_connection(int fd, struct sockname *usernames) {
    ...
    if(user_read == -1)
    {
        perror("Could not load username");
        return -1;
    }
    usernames[user_index].username = strdup(buf);
...
int read_from(int client_index, struct sockname *usernames) {
    ...
    strcpy(prefix, usernames[client_index].username);
    strcat(prefix, ": ");
    char to_write[BUF_SIZE*2]; strcpy(to_write,prefix); strcat(to_write, buf);
